...
const COLOR_SCHEME = {
  ...
  dark: {
    particle: {
      color: "rgba(16, 185, 129, 0.35)", // Subtle emerald glow matching theme
    },
...
export const FluidParticlesBackground = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      const isDark = true; // Dashboard is solid dark executive theme
      const scheme = isDark ? COLOR_SCHEME.dark : COLOR_SCHEME.light;

      // Clear canvas with subtle semi-transparent background to create organic fluid trails
      ...
      for (const particle of particles) {
        ...
        const opacity =
          Math.sin((particle.life / particle.maxLife) * Math.PI) * 0.28; // Subtle atmospheric glow
